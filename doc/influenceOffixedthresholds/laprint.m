...
% 	   ', ''options'', LAPRINTOPT)'];
  ...
  case 'whois'
    msgbox({['This is LaPrint, Version ' laprintident],...
          ...
          'LaPrint Info')
%    case 'textwidth'
%     LAPRINTOPT.textwidth = textwidth;
   otherwise 
...
cmd = strrep(printcmd,'<filename.eps>',epsbasenameext);
...
if ~scalefonts
  if (exist('textwidth'))
    switch package
     case 'epsfig'   
      fprintf(fid,[ '\\epsfig{file=' epsfullnameext ',width=' ...
                    num2str(textwidth) '\\textwidth}%%\n' ]);
     case 'graphicx'   
      fprintf(fid,[ '\\includegraphics[width=' num2str(textwidth) ...
                    '\\textwidth]{' epsfullnameext '}%%\n' ]);
     otherwise  
  ...
  if (exist('textwidth'))
    switch package
     case 'epsfig'  
      fprintf(fid,[ '\\resizebox{' num2str(textwidth) '\\textwidth}{!}' ...
                    '{\\epsfig{file=' epsfullnameext '}}%%\n' ]);
     case 'graphicx' 
      fprintf(fid,[ '\\resizebox{' num2str(textwidth) '\\textwidth}{!}' ...
                    '{\\includegraphics{' epsfullnameext '}}%%\n' ]);
     otherwise
      warning('LaPrint:general',['Package ''' package ''' not known. '...
                          'I hope you know what you are doing...'])    
    end
...
if createview | processview
  ...
  if head
    ...
    fprintf(fid,[ '%% It calls ' texbasenameext ...
		  ', which in turn  calls ' epsfullnameext '.\n' ]);
