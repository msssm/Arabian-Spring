...
% A graph with n*k/2 edges is constructed, i.e. the nodal degree is n*k 
% for every node.
%
% INPUT
...
%
% OUPUT
% A: [n n] sparse symmetric adjacency matrix representing the
% generated graph
...
columns = rows+reshape(repmat([[1:par.kHalf(ni)] ...
    [par.nodes(ni)-par.kHalf(ni):n-1]], par.nodes(ni), 1), ...
    par.nodes(ni)*k, 1);
columns = mod(columns-1, par.nodes(ni)) + 1;
B = sparse(rows, columns, ones(par.nodes(ni)*k, 1));
A = sparse([], [], [], par.nodes(ni), par.nodes(ni));

% With probability beta rewire an edge avoiding loops and link 
% duplication. Until step i, only the columns 1:i are generated making 
% implicit use of A's symmetry.
for i = [1:par.nodes(ni)]

% The i-th column is stored full for fast access inside the following
% loop.
...
% A is not yet symmetric: to speed things up, an edge connecting i 
% and j, i < j implies A(i,j)==1, A(i,j) might be zero.
T = triu(A);
A = T+T';
